    ...
    private void ProcessPlotCommands()
    {
        ...
        foreach (var plotCmd in _plotCommands)
        {
            try
            {
                ...
                // _plottingService.CreatePlot will generate PlotData and raise its PlotCreated event.
                // Since StatementExecutor.PlotCreated forwards this, MainWindowViewModel will receive it.
                var plotData = _plottingService.CreatePlot(plotCmd.CommandText, _integralTable);

                if (plotData == null) // CreatePlot might return null if it fails internally
                {
                    Console.WriteLine($"Warning: PlotData was not generated by PlottingService for command: {plotCmd.CommandText}.");
                    continue;
                }
                // No need to explicitly invoke PlotCreated here in StatementExecutor
                // as it's handled by the event forwarding from _plottingService.
                Console.WriteLine($"Plot data generated for command: {plotCmd.CommandText} (event forwarded by PlottingService)");
            }
            catch (Exception ex)
            {
